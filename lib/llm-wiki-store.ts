...
export async function buildLlmWikiGraph(input: LlmWikiBuildInput) {
  ...
  for (const idea of ideas) {
    ...
    if (idea.analysis?.suggestedProject) {
      addEdge(edges, { from: nodeId('idea', idea.slug), to: nodeId('project', idea.analysis.suggestedProject), type: 'becomes', reason: 'Idea analysis suggests this project target.', confidence: 'medium' });
    }
