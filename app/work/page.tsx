  ...
  return <div className="grid gap-5">
    <Panel dark className="console-screen relative min-h-[380px] overflow-hidden p-6 md:p-8">
      ...
      <div className="relative flex h-full flex-col justify-between gap-10">
        <div>
          <Label>Public Work / Pigou Workshop</Label>
          <h2 className="mt-8 max-w-[10ch] text-5xl font-semibold leading-[.92] text-white md:text-8xl">Things I Built</h2>
        </div>
      </div>
    </Panel>
    ...
    <section className="grid gap-5 lg:grid-cols-3">
      {featured.map(project => <Panel key={project.slug} raised className="overflow-hidden p-0">
        ...
        <div className="p-5">
          ...
          <Link href={`/projects/${project.slug}`} className="mono mt-4 inline-flex min-h-10 items-center rounded-full border border-[var(--border-visible)] px-4 text-[10px] uppercase text-[var(--text-primary)]">view public page</Link>
        </div>
      </Panel>)}
    </section>

    <Panel className="p-5 md:p-6">
      <SectionHeader label="公开项目" value={`${projects.length} item(s)`} />
      <div className="grid gap-5">
        {projects.map(project => {
          const publicImages = project.images?.filter(image => image.public).slice(0, 2) || [];
          return <div key={project.slug} className="grid gap-5 border-b border-[var(--border)] pb-5 last:border-b-0 lg:grid-cols-[.8fr_1.2fr]">
            <div>
