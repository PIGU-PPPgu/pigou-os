...
function extractWebhookInput(body: Record<string, unknown> | null) {
...
async function readInboxBody(request: Request) {
...
function isInboxAuthorized(request: Request) {
  if (isAuthenticatedRequest(request)) return true;

  const secret = webhookSecret();
  if (!secret) return false;

  const provided = request.headers.get('x-pigou-inbox-secret') || getBearerToken(request);
...
export async function POST(request: Request) {
  if (!isInboxAuthorized(request)) {
    return NextResponse.json({ ok: false, message: '请先登录 Pigou OS，或为 webhook 配置 PIGOU_INBOX_WEBHOOK_SECRET。' }, { status: 401 });
  }
  ...
  const draft = applyProjectTags(applyOverrides(classifyInboxInput(input, requestedMode), body || {}), input, projects);
